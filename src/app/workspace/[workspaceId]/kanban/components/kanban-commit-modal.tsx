...
interface KanbanCommitModalProps {
  ...
  fileCount: number;
  /** VCS type — adjusts copy and flow for Git vs SVN */
  vcsType?: "git" | "svn" | "none";
}

export function KanbanCommitModal({ open, onClose, onCommit, fileCount, vcsType = "git" }: KanbanCommitModalProps) {
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              {isSvn ? "SVN Commit" : "Create Commit"}
            </h2>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {isSvn
                ? `Commit ${fileCount} file${fileCount === 1 ? "" : "s"} to repository (SVN commit pushes immediately)`
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            >
              ...
              {loading
                ? (isSvn ? "Committing & Pushing..." : "Committing...")
                : (isSvn ? "Commit & Push" : "Commit")}
            </button>
          </div>
        </form>
      </div>
