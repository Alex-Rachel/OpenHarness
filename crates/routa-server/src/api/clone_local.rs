...
//!
//! POST /api/clone/local - Validate and load an existing local directory (Git, SVN, or plain)

use axum::{routing::post, Json, Router};
use serde::Deserialize;

use crate::api::repo_context::{normalize_local_repo_path, validate_local_repo_path};
...
async fn load_local_repo(
    ...
    let vcs_type = validate_local_repo_path(&repo_path)?;
    ...
    // For Git repos, return full branch and status info
    ...
    // For SVN and non-VCS directories, return basic info
    Ok(Json(serde_json::json!({
        ...
        "branch": null,
        "branches": [],
        "status": { "isRepo": false, "branch": "", "modified": [], "staged": [], "untracked": [] },
    })))
}
